...
# 구조:
#   - A_crawl_drive.CrawlDriver는 사용하지 않는다.
#   - contest.py 내부에 ContestQuadDriver를 따로 둔다.
#   - ContestQuadDriver는 A_quad_api.make_default_api()만 사용해서
#     직접 다리 xyz 명령을 보낸다.
#
# 기본 동작:
#   - 계속 직진
...
# AR마커 화면 면적 기준. 값이 클수록 더 가까이 왔을 때 반응
TRIGGER_AREA = 8000
...
# 한 번 직진 step에서 swing 다리를 보내는 거리
STEP_FWD = 40.0

# 제자리 회전 시 다리 앞/뒤 이동량
STEP_YAW = 60.0

# 90도 회전에 필요한 회전 step 수. 실제 로봇에서 튜닝 필요
TURN_90_STEPS = 4
...
def side_sign(leg_id: int) -> int:
    """
    오른쪽 다리 +1, 왼쪽 다리 -1.
    제자리 회전에서 좌/우 다리를 반대로 움직이기 위해 사용.
    """
    return +1 if leg_id in RIGHT_LEGS else -1
...
# ============================================================
# 6. contest.py 전용 4족 보행 드라이버
# ============================================================
class ContestQuadDriver:
    """
    A_quad_api만 사용해서 동작하는 contest 전용 드라이버.

    A_crawl_drive.CrawlDriver를 가져오지 않기 때문에
    키보드 입력 처리, 방향 전환 reset, fb_step 같은 상위 로직과 충돌하지 않는다.
    """
    ...
    def forward_step(self):
        """
        단순 직진 step.
        한 다리를 들어 앞으로 보내고, 나머지 다리는 기준 자세로 유지한다.
        """
        leg_id = FORWARD_SEQ[self.forward_idx % len(FORWARD_SEQ)]
        self.forward_idx += 1
        ...
        if not self.set_pose(leg_id, x0, y0, z_lift, PHASE_T):
            self.go_stand(0.3)
            return

        if not self.set_pose(leg_id, x0 + dx, y0, z_lift, PHASE_T):
            self.go_stand(0.3)
            return

        if not self.set_pose(leg_id, x0 + dx, y0, sz, PHASE_T):
            self.go_stand(0.3)
            return

        # step 누적을 줄이기 위해 한 step 뒤에는 기준 자세로 복귀
        self.go_stand(duration=0.15)
    ...
    def turn_step(self, direction: int):
        ...
        # 좌/우 다리를 반대 방향으로 보내 제자리 회전 효과를 만든다.
        body_dx = direction * STEP_YAW * side_sign(leg_id)
        dx = body_x_to_local_x(leg_id, body_dx)
...
class ContestMission:
    ...
    def handle_marker(self, m: MarkerInfo):
        print(f"[MARKER] ID={m.marker_id} AREA={int(m.area)}", flush=True)
        ...
        if m.marker_id == 1:
            ...
            print(f"[MARKER] ID={m.marker_id}는 미션 대상이 아니므로 무시", flush=True)
            return
